...
export class User {
  ...
  phone;
  name;
  image; // Todos os atributos úteis retornados
  ...
  updateList(token, attributes = [{ name: null, value: null }]) {
    ...
    attributes.forEach((column) => {
      // itera colunas a serem atualizadas
      try {
        this.update({
        ...
        errors.push(
          // Guarda exceção para ser retornada
   ...
   */
  async update(data) {
    let response = await api.post(`${this.url}/update`, data);
   ...
   */
  async delete(token) {
    let response = await api.get(`${this.url}/delete`, { id: token });
