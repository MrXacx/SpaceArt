...
export class Chat{
    ...
    async create(){
        ...
        if(response !== status.CREATED){
            error.HTTPRequestError.throw("Não foi possível enviar uma mensagem para o banco");
        }
    }
    ...
    async fetch(){
        ...
        if(response.status !== status.OK){
            error.HTTPRequestError.throw(`Não foi possível buscar a conversa ${this.id}`);
        }
    ...
    async fetchList(user, offset = 0, limit = 10){
        let response = await request.get(`${this.url}?offset=${offset}&limit=${limit}user=${user}`);

        if(response.status !== status.OK){
            error.HTTPRequestError.throw("Não foi possível buscar uma lista de conversas");
        }
    ...
    async fetchMessages(offset = 0, limit = 10){
        let response = await request.get(`${this.url}/message?offset=${offset}&limit=${limit}&chat=${this.id}`);

        if(response.status !== status.OK){
            error.HTTPRequestError.throw(`Não foi possível buscar as mensagens do chat ${this.id}`);
        }
